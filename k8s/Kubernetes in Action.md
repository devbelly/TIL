...
- 스케줄러가 노드를 선택하는 기준은 request
- 실제 사용량에 따라 스케줄링하면 기존에 실행되는 파드에 영향을 미치므로 이렇게 해선 안된다
- 우선순위를 부여할때 LeastRequestPriority(비어있는 공간이 많음)와 MostRequestPriority(비어있는 공간이 적음)가 있다.
	- 클라우드 환경에서는 실행되는 노드에 파드를 꽉꽉 담는것이 중요하므로 MostRequestPriority를 사용해야한다.

## 14.2 컨테이너에 사용가능한 리소스 제한
- 제한만 설정하면 요청은 자동으로 제한과 동일해진다.
- CPU는 압축가능한 리소스이다
	- 파드 내부에서 실행되는 컨테이너에 영향을 미치지 않고 CPU 사용량을 조절할 수 있다는 의미이다.
- 메모리는 압축가능하지 않은 리소스이다.
	- 컨테이너 프로세스가 사용하는 메모리를 해제하지 않는 한 해당 메모리를 다시 가져올수 없다는 의미이다.
	- 메모리 때문에 리소스 제한을 설정해야한다. 안그러면 노드 내부의 다른 파드와 스케줄링되는 파드에 영향을 미치기 때문이다.

## 14.3 Qos

- 리소스 요청 + 제한을 어떻게 설정했느냐에 따라서 파드의 Qos가 설정된다. 이는 파일에 직접 선언하는 것이 아니라 앞에 언급한 설정에 따라 자동으로 파드에 할당된다.
- BestEffort, Burstable, Guaranted로 나눌 수 있다.
- 리소스가 오버커밋된 상황에서 Qos에 따라 종료될 파드를 선택한다.
- 만약 동일한 레벨의 Qos만 존재한다면 실제 메모리 사용비율(실사용률/리소스요청량)에 따라 사용비율이 높은 것 부터 종료된다.


## 14.4 limitRange & ResourceQuota

- 클러스터의 제한된 자원을 네임스페이스를 정의해서 나눠갖는다.
- 하나의 네임스페이스에서 자원을 많이 차지하면 다른 네임스페이스에 영향을 미치게 되므로 ResourceQuota를 지정해서 하나의 네임스페이스 사용할 수 있는 자원량을 제한한다.
	- 쿼터 범위를 지정해서 더 범위를 좁힐수도 있다.(BestEffort, NotBestEffort, Terminating, NonTerminating)
- limitRange는 파드나 오브젝트에 할당되는 resource를 검사하거나 지정되어있지 않다면 기본 resource를 지정해줄 수 있다.
	- limitRanger 어드미션 컨트롤러에 의해 이루어진다.


# 15장.

#### 오토스케일링
- 파드의 오토스케일링은 아래 단계를 따른다.
	- 파드의 메트릭 모으기
	- 지정한 목표값과 비슷하게 레플리카 계산
		- 이때 기준을 여러가지로 설정할 수 있다.(cpu 사용량, 메모리 사용량 등)
		- cpu 사용량 자체도 관점에 따라 다를 수 있다.(실 사용률, 요청량 등)
		- 기준을 요청량으로 하므로 `limitRange`설정이 직간접적으로 필요하다.
	- replicas 필드 갱신하기

#### 파드 메트릭 얻기
- 각 노드에 `kubelet`가 존재. 이 안에 `cAdvisor`가 들어가서 각 노드마다 메트릭을 측정한다.
- 클러스터 단위의 메트릭을 얻기 위해 힙스터로 정보를 보내고
- 힙스터가 HPA로 정보를 전달한다.

#### 스케일링 세부사항
- 지정한 목표값에 따라 레플리카를 계산하지만 한번에 스케일되는 파드 수에는 제한이 있다.
	- 파드수가 3개 이상이라면 최대 두배의 파드만 생성될 수 있다.
	- 파드수가 1,2 개라면 최대 4개의 파드까지 늘어날 수 있다.
	- 만약 파드가 1개인 상황에서 계산치에 따라 5개의 파드가 생성되어야 계산했더라도 최대 4개까지 생성
- 스케일 이벤트가 발생하고나서 일정시간이 지난 후에 스케일 이벤트가 발생한다.
	- up은 3분 down은 5분

#### 메트릭 기준 및 고려할 점
- 리소스, 오브젝트, 파드를 사용할 수 있다.
- 책에서 언급한 cpu 리소스는 간단하지만 메모리는 cpu에 비해서 복잡하다.
- 오토스케일링 시 메트릭 기준이 선형에 가깝게 감소할 것을 기대해야한다.
	- cpu나 qps는 파드를 늘린만큼 감소할 것이라 예측하기 쉽다.
	- 메모리는 파드를 늘렸다고 해서 감소하지는 않는다. → 메모리는 기준으로 적당하지 않다.

# 17장. 애플리케이션 개발을 위한 모범사례

#### 로컬 IP와 호스트 이름 변경 예상하기
- 파드가 재시작되면 IP와 호스트의 이름이 변경된다.
- 호스트의 이름 변경을 원하지 않는다면 스테이트풀셋을 사용할 수 있다 (이 경우에도 IP는 변경된다)

#### 디스크에 기록된 데이터가 사라지는 경우 예상하기
- 파드가 재시작되면 기록된 내용이 사라진다.
- 파드가 재시작되지 않더라도 기록된 내용이 사라질 수 있다.
	- 프로세스 크래시, OOM 문제로 컨테이너가 재시작되면 쓰기 레이어에 기록된 내용이 사라진다.
- 프로세스가 다시 시작할 때 파드가 가지고 있는 내용을 보존하기 위해 볼륨을 사용할 수도 있다.
- 볼륨을 사용하는 것은 양날의 검이므로 신중하자
	- 기록된 내용이 잘못되어 컨테이너가 재시작되는 상황이 있을 수 있기 때문이다.

#### 초기화 컨테이너
- 디플로이먼트에서 파드를 실행할 때 실행되는 순서를 지정하지는 않는다.
- 순서를 강제하고 싶다면 초기화 컨테이너를 활용할 수 있다.
- 파드의 주 컨테이너 시작전에 실행되는 컨테이너로써 볼륨을 초기화하는데 사용한다.
